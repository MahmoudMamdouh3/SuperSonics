...
if __name__ == "__main__":
    ...
    parser.add_argument("--type", type=str, default="noisy", help="Either 'speech', 'noise' or 'noisy'.")
    parser.add_argument(
        "--audio_files",
        type=str,
         default="D:/CMGAN-main/CMGAN-main/DS/new/New Text Document.txt",

        help="Text file containing speech or noise files separated by a new line.",
    )
    parser.add_argument(
        "--hdf5_db", type=str,
          default="D:/Grad/DeepFilterNet/DeepFilterNet-main/DeepFilterNet-main/DeepFilterNet/newDS/",
          help="HDF5 file name where the data will be stored in."
    )
    ...
    with open(args.audio_files) as f:
         ...
         res = []
         for file in f:
          res.append(_check_file(file))
         data[args.type]["files"] = res
